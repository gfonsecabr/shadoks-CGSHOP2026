...
public:
  Segment() :
    ...
    {}
  Segment(const Point<Number> &_s, const Point<Number> &_t) :
    s(_s), t(_t)
    {}
  Point<Number> source() const {return s;}
  Point<Number> target() const {return t;}

  Segment<Number> operator*(Number k) const {
    return Segment(s*k, t*k);
  }

  Segment<Number> operator+(Point<Number> p) const {
    return Segment(s+p, t+p);
  }
  ...
  auto operator<=>(const Segment<Number> &seg) const {
  ...
  bool adjacent(const Segment<Number> &seg) const {
    return incident(seg.source()) || incident(seg.target());
  }

  Intersection intersect(const Segment<Number> &seg) const {
  ...
  bool interiorDisjoint(const Segment<Number> &seg) const {
    int o1 = orientation(seg.source());
    int o2 = orientation(seg.target());
    if(o1 == 0 && o2 == 0)
  ...
  Segment<Number> sorted() const {
    if(source() < target())
      return *this;
    return Segment<Number>(target(), source());
  }

  bool cross(const Segment<Number> &seg) {
    int o1 = seg.orientation(s);
    if(o1 == 0)
      return false;
    int o2 = seg.orientation(t);
    if(o1 == o2 || o2 == 0)
      return false;

    o1 = orientation(seg.s);
    if(o1 == 0)
      return false;
    o2 = orientation(seg.t);
    if(o1 == o2 || o2 == 0)
      return false;
...
template <class Number>
std::ostream &operator<<(std::ostream &os, Segment<Number> const &seg) {
  return os << seg.source() << "->" << seg.target();
}

namespace std {
  template <class Number> struct hash<Segment<Number>> {
    size_t operator()(const Segment<Number> &t) const {
      size_t seed = 0;
      for(auto p : {t.s,t.t}) {
